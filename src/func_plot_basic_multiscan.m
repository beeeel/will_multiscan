function [figure_handles]=func_plot_basic_multiscan(data,axis_info,filename,exp_params,plot_params)
...
y_sel =1;
    ...
    for k=1:length(data.shifted);
        ...
        if data.dc_count>0
            subplot(3,3,1);imagesc(axis_info.axis2.um,axis_info.axis1.um,data.dc{k});%axis image;
            a = get(gca,'position');c =colorbar('location','westoutside');set(gca,'position',a);title(c,'V')
            set(gca,'interruptible','off','BusyAction','cancel','ButtonDownFcn', {@func_plot_basic_callback_multiscan,plotting_vars});                 %assign call back function when mouse clicked in figur
            set(get(gca,'Children'),'interruptible','off','BusyAction','cancel','ButtonDownFcn', {@func_plot_basic_callback_multiscan,plotting_vars}); %apply to data in the figure as well
            
            title(sprintf('DC%d',k));
            hold on;plot(axis_info.axis2.um(y_sel),axis_info.axis1.um(x_sel),'wx','markersize',8,'linewidth',2);hold off
        end
        
        subplot(3,3,4);imagesc(axis_info.axis2.um,axis_info.axis1.um,data.f_amp{k}.*a_mask);%axis image;
        ...
        subplot(3,3,7);imagesc(axis_info.axis2.um,axis_info.axis1.um,data.freq{k}.*f_mask,[exp_params.f_min exp_params.f_max]);%axis image;
        a = get(gca,'position');c=colorbar('location','westoutside');set(gca,'position',a);
        set(gca,'interruptible','off','BusyAction','cancel','ButtonDownFcn', {@func_plot_basic_callback_multiscan,plotting_vars});                 %assign call back function when mouse clicked in figur
        set(get(gca,'Children'),'interruptible','off','BusyAction','cancel','ButtonDownFcn', {@func_plot_basic_callback_multiscan,plotting_vars}); %apply to data in the figure as well
        ...
        xlabel('ns');title(sprintf('Raw trace:%1.2f,%1.1f',axis_info.axis1.um(x_sel),axis_info.axis2.um(y_sel)));
