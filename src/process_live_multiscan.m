...
% ******************************************************************************************************************
ofst = 0;
exp_params.default_loc = 275;          % where the co peak should be found
exp_params.start_offset = 90+ofst;           % off set from co peak location for data selection
exp_params.co_peak_thresh = 1e-5;       % co peak level in volt for ac or mod depth for mod data
exp_params.trace_length=1500-ofst;           % how many points to use in trace for basic processing
exp_params.fit_order = 14;               % thermal removal fit order
exp_params.f_min = 5;
exp_params.f_max = 6;
exp_params.co_peak_range = (-100:100)+exp_params.default_loc;   %search range for co_peak
exp_params.forced_co_peak = 'no';       % force to use default loc for co peak
exp_params.LPfilter = 30;               % in GHz
...
[fname, axinf]=func_get_scan_details_multiscan(filename, true);

for sidx = 1:axinf.number_of_scans
    sName = sprintf('scan%i',sidx);
    dl(sidx) = dir(fname.(sName).ac);
end

[~, newidx] = max([dl.datenum]);
filename = fname.(sprintf('scan%i',newidx));
axis_info = axinf.(sprintf('scan%i',newidx));
if ~any(newidx == [1 12])
    [~, yax] = max(axis_info.axis_pts(1:2));
    axis_info.xpts = axis_info.axis_pts(3);
    axis_info.ypts = axis_info.axis_pts(yax);
else
    axis_info.xpts = axis_info.axis_pts(1);
    axis_info.ypts = axis_info.axis_pts(2);
end
...
first_run=1;
lastidx = newidx;
while(currentfileSize~=filesize_final || newidx ~= length(dl))
    for sidx = 1:axinf.number_of_scans
        sName = sprintf('scan%i',sidx);
        dl(sidx) = dir(fname.(sName).ac);
    end
    ...
    if ~any(newidx == [1 12])
        ...
        axis_info.xpts = axis_info.axis_pts(3);
        axis_info.ypts = axis_info.axis_pts(yax);
        axis_info.y_um = axis_info.(sprintf('axis%i',yax)).um;
        axis_info.x_um = linspace(0,range(axis_info.y_um),axis_info.xpts);
    else
        axis_info.xpts = axis_info.axis_pts(1);
        axis_info.ypts = axis_info.axis_pts(2);
        axis_info.x_um = axis_info.axis1.um;
        axis_info.y_um = axis_info.axis2.um;
    end
    axis_info.no_points = axis_info.points_per_trace;
    ...
    fileInfo = dir(filename.meta.dataname);
    ...
    for k=1:traces_change;
        ...
        if first_run==1;
            ...
            for P = 1:filename.meta.n_channels;
                data.ac{P}=zeros(axis_info.xpts,axis_info.ypts,axis_info.points_per_trace);
                if isfield(exp_params,'allow_dc')
                    if exp_params.allow_dc==1
                        data.dc{P}=zeros(axis_info.xpts,axis_info.ypts);
                        data.mod{P}=zeros(axis_info.xpts,axis_info.ypts,axis_info.points_per_trace);
                    end
                end
